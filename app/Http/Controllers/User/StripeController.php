...
//             $total_amount = Session::get('coupon')['total_amount'];
//         }else{
//             $total_amount =intval(Cart::total());
//         }
...
public function CashOrder(Request $request){
    ...
    if(Session::has('coupon')){
        ...
        // Validate if total_amount is numeric
        if (!is_numeric($total_amount)) {
            return redirect()->back()->with('error', 'Invalid total amount in coupon');
        }
    } else {
        $total_amount = Cart::total();

        // Clean the total amount string to remove any non-numeric characters
        $total_amount = preg_replace('/[^\d.]/', '', $total_amount);

        // Ensure Cart::total() is numeric before converting
        if (!is_numeric($total_amount)) {
